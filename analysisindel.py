...
perfect_match_requirement = input('enter the length of fragments for analysis (default is 25bp):')
if not perfect_match_requirement:
    perfect_match_requirement = 25
else:
    perfect_match_requirement = int(perfect_match_requirement)
...
for file in os.listdir(directory):
    ...
    if filename.endswith(".sam"):
        ...
        wanted_labels = list(filter(unwanted_labels, unique_labels))
        possible_indel_rows = df.loc[df['CIGAR'].isin(wanted_labels)]
...
#output.to_excel('indel_count(full).xlsx',index = False)
...
df_final.to_csv('transposed_frame(full).csv',index = False)
