...
for file in os.listdir(directory):
    ...
    kmer_for_alignment = open('%s_highc_kmer.fasta'%sample_name,'w')
    ...
    for record in records:
        ...
        if length_of_seq >= end_site:
            for i in range(0, length_of_seq-25, 5):
    ...
    os.system('bwa/bwa aln ref.fasta %s_highc_kmer.fasta > %s_highc_kmer.sai' % (sample_name,sample_name))
    os.system('bwa/bwa samse ref.fasta %s_highc_kmer.sai %s_highc_kmer.fasta > aln_%s_highc_kmer.sam' % (sample_name,sample_name,sample_name))

    '''generate tdf
    runcmd("aln_"+sample_name+'_highc'+'_kmer')
    '''
