...
'''a demo for single file analysis is available in jupyter notebook (handle_sam.jpynb)
    ...
    line 125 are names of the leaves.
'''
...
for file in os.listdir(directory):
    ...
    if filename.endswith(".sam"):
        ...
        df = pd.read_csv('aln_%s_highc_kmer.sam'%leaf_name,skiprows = 2,delim_whitespace=True,header=None, index_col=None,names =  ['QNAME','FLAG','RNAME','POS','MAPQ','CIGAR','RNEXT','PNEXT','TLEN','SEQ','QUAL','M1','M2','M3','M4','M5','M6','M7','M8','M9'])
        df = df.iloc[:, : 10]
        df['protospacer'] = 'Null'
...
#if you want to use this function, unmute line 96, 97 and line 109
...
'''
output.to_csv('w_t_high_c_indel_count_from_sam_files_ordered_by_plants.csv',index = False)
...
df_final.to_csv('w_t_high_c_indel_count_from_sam_files_ordered_by_mutation_types.csv',index = False)
